...
setTimeout(() => {
  ...
  console.log(
    `   🎯 Has non-zero values: ${hasNonZeroValues ? "YES (not fresh)" : "NO (fresh state)"}`,
  );

  console.log("\n💡 Reset Instructions:");
  console.log(
    "   1. UI Method: Settings → ডেটা ম্যানেজমেন্ট → 'সম্পূর্ণ রিসেট করুন'",
  );
    ...
    if (window.performCompleteReset) {
      ...
      setTimeout(() => {
        if (window.verifyResetComplete) {
          ...
          console.log(
            `🎯 Reset verification: ${isClean ? "SUCCESS" : "INCOMPLETE"}`,
          );
        }
