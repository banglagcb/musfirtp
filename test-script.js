...
async function testApplication() {
  ...
  const usernameField = document.querySelector(
    'input[placeholder*="জারনেম"], input[placeholder*="Username"]',
  );
  const passwordField = document.querySelector('input[type="password"]');
  const loginButton = document.querySelector(
    'button[type="submit"], button:contains("লগইন"), button:contains("Login")',
  );
  ...
  if (languageButton) {
    ...
    const englishText = document.querySelector(
      '*:contains("Dashboard"), *:contains("Bookings")',
    );
    if (englishText) {
  ...
  console.log(
    `Current screen width: ${window.innerWidth}px (${isMobile ? "Mobile" : "Desktop"})`,
  );

  // Test 7: Check for missing translations
  console.log("📖 Checking for missing translations...");
  const bengaliTexts = document.querySelectorAll(
    '*:contains("ড্যাশবোর্ড"), *:contains("বুকিং"), *:contains("রিপোর্ট")',
  );
  const englishTexts = document.querySelectorAll(
    '*:contains("Dashboard"), *:contains("Bookings"), *:contains("Reports")',
  );
