...
setTimeout(() => {
  ...
  const notificationElements = document.querySelectorAll(
    '[class*="TicketInventoryNotification"]',
  );
  console.log(`📋 Found ${notificationElements.length} notification elements`);

  // Look for notification content
  const ticketNotifications = document.querySelectorAll(
    'div[class*="fixed bottom-4 right-4"]',
  );
  console.log(
    `🔍 Found ${ticketNotifications.length} bottom-right positioned elements`,
  );
  ...
  if (window.ticketNotificationService) {
    ...
    console.log(
      `🌍 Managing ${countriesData.length} countries:`,
  ...
  const ticketElements = document.querySelectorAll(
    '*:contains("টিকেট স্টক"), *:contains("Ticket Stock")',
  );
  const countryElements = document.querySelectorAll(
    '*:contains("দুবাই"), *:contains("Dubai"), *:contains("দোহা"), *:contains("Doha")',
  );
  ...
  setTimeout(() => {
    ...
    if (newNotifications.length > 0) {
      ...
      console.log(
        "⚠️  No notifications visible yet - they appear every 3 minutes",
      );
    }

    // Test language switching impact
    const languageButton = document.querySelector(
      'button[title*="Switch"], button[title*="পরিবর্তন"]',
    );
    if (languageButton) {
...
console.log(
  "💡 Run window.monitorNotifications() to watch for new notifications",
);
