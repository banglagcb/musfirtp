...
setTimeout(() => {
  ...
  console.log(
    `   📋 Bookings: ${currentBookings ? JSON.parse(currentBookings).length : 0} records`,
  );
  console.log(
    `   👥 Users: ${currentUsers ? JSON.parse(currentUsers).length : 0} users`,
  );
  ...
  settingsButtons.forEach((button) => {
    if (
      button.textContent?.includes("সম্পূর্ণ রিসেট") ||
