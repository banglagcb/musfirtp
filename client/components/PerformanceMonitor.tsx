...
const PerformanceDashboard = memo(({ isOpen, onClose }: PerformanceDashboardProps) => {
  const { theme } = useApp();
  ...
  const formatScore = (score: number) => {
    ...
    if (score < 300) return { color: 'text-yellow-500', label: 'Needs Improvement' };
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className={cn(
              ...
              <div className={`text-xs ${formatScore(vitals.cls * 1000).color}`}>
              ...
              {Object.entries(metrics).map(([name, data]) => (
                <div key={name} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-mono">{formatTime(data.average)}</div>
                    <div className={cn(
                      "text-xs",
