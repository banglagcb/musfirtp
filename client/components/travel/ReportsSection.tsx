...
export default function ReportsSection({
  ...
  const reportStats = useMemo((): ReportStats => {
    ...
    const topAirline =
      Object.entries(airlineCount).sort(([, a], [, b]) => b - a)[0]?.[0] ||
      "N/A";
    ...
    const topRoute =
      Object.entries(routeCount).sort(([, a], [, b]) => b - a)[0]?.[0] || "N/A";
  ...
  const airlineStats = useMemo(() => {
    const stats: Record<
      string,
      { bookings: number; revenue: number; profit: number }
    > = {};
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">
                +{Math.round(reportStats.conversionRate)}% সম্পূর্ণ
              </p>
            </div>
              ...
              <p className="text-xs text-gray-500">
                ...
                {Math.round(
                  reportStats.totalRevenue /
                    Math.max(reportStats.totalBookings, 1),
                ).toLocaleString()}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                ৳{reportStats.totalProfit.toLocaleString()}
              </p>
              <p className="text-xs text-gray-500">
                {reportStats.profitMargin.toFixed(1)}% মার্জিন
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600">
                ৳{Math.round(reportStats.averageProfit).toLocaleString()}
              </p>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-blue-600">
                    ৳
                    {Math.round(
                      airline.profit / airline.bookings,
                    ).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3">
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${(airline.revenue / Math.max(...airlineStats.map((a) => a.revenue))) * 100}%`,
                    }}
        ...
        {timeSeriesData.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              নির্বাচিত সময়কালে কোনো ডেটা পাওয়া যায়নি
            </p>
          </div>
        )}
      </motion.div>
